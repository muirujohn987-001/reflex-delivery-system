...
export default function Select({ label, error, options = [], placeholder = "Select an option", className = "", containerClassName = "", ...props }) {
  const id = useId();
  return (
      ...
      <div className="relative">
        <select
          ...
          className={`w-full min-h-[44px] appearance-none rounded-xl border bg-white px-3.5 py-2.5 pr-10 text-sm text-ink
            ...
            ${error ? "border-red-300 focus:border-red-400 focus:ring-red-100" : "border-gray-100 focus:border-teal-400 focus:ring-teal-100"}
